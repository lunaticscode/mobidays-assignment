...
```
features/filter/ui/GlobalFilter.tsx       # 프레젠테이션
features/filter/hooks/useGlobalFilter.ts  # 필터 상태 관리 훅
```
...
```
features/chart/ui/DailyStatsGraph.tsx        # 차트 프레젠테이션
features/chart/hooks/useDailyStats.ts        # 일별 데이터 조회 훅
```
...
```
features/campaign/ui/CampaignTable.tsx          # 테이블 프레젠테이션
features/campaign/hooks/useCampaigns.ts         # 캠페인 목록 조회 훅
features/campaign/hooks/useBulkStatusUpdate.ts  # 일괄 상태 변경 훅
```
...
| 집행금액 | cost 합계 | O | `₩{금액}` 포맷, 천단위 콤마 |
| CTR (%) | clicks / impressions × 100 | O | 소수점 2자리 |
| CPC (원) | cost / clicks | O | 정수, 천단위 콤마 |
| ROAS (%) | conversionsValue / cost × 100 | O | 소수점 2자리 |

### 계산 지표 (daily_stats 합산 기준)

```
CTR (%) = (총 클릭수 / 총 노출수) × 100
CPC (원) = 총 비용 / 총 클릭수
ROAS (%) = (총 전환가치 / 총 비용) × 100
```
...
```
features/campaign/ui/CampaignRegisterModal.tsx   # 모달 프레젠테이션
features/campaign/form/campaign-register.ts      # Zod 스키마 + 타입
features/campaign/hooks/useCreateCampaign.ts     # 등록 Mutation 훅
```
